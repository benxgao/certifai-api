...
> = async (req, res): Promise<void> => {
  try {
      ...
      if (
        ...
        status =
          examFromDb.score >= examFromDb.certification.pass_score
            ...
            : 'SCORE_BELOW_THRESHOLD'; // TODO: exam status refactoring
      } else {
        status = ExamStatus.COMPLETED; // Submitted but score or pass_score is not available
      }
    } else if (examFromDb.exam_status === ExamStatus.READY && examFromDb.started_at) {
    ...
    if (examFromDb.exam_status === ExamStatus.QUESTIONS_GENERATING) {
      try {
